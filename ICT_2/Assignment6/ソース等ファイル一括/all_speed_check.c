...
// 平文 (32byte)
  unsigned char message[] = "This is a massage for speed test";
  unsigned char *ptext;
...
//DES,AES用変数
  EVP_CIPHER_CTX  ctx;
  int cipher_len, message_len, enc_len, f_len=0;
...
//planeDataを公開鍵で暗号化。パディング方式は、PKCS #1 v1.5 パディングを指定
  for (i=0; i<100000; i++){
    if( RSA_public_encrypt(strlen(message), message, enc, rsaKey, RSA_PKCS1_PADDING) < 0 ) {
      free(enc);
...
// 2Key T-DES復号処理
  for (i=0; i<100000; i++){
    message_len=enc_len+EVP_MAX_BLOCK_LENGTH;  // 復号データの長さを仮置き（Byte)
    dec=(unsigned char *)calloc(message_len,sizeof(char));
    EVP_CIPHER_CTX_init(&ctx);
    EVP_DecryptInit_ex(&ctx, EVP_des_ede(), NULL, (unsigned char*)key_16, NULL);
    EVP_DecryptUpdate(&ctx, dec, &message_len, (unsigned char *)enc, cipher_len+f_len);
    EVP_DecryptFinal_ex(&ctx,(unsigned char *)(dec+message_len),&f_len);
    EVP_CIPHER_CTX_cleanup(&ctx);
  }
  end_time =  clock();
  run_time = (double)(end_time - first_time) / (double)CLOCKS_PER_SEC;
