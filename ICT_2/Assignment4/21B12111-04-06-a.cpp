...
// プログラミング言語：C++
// コンパイル方法：g++ 21B12111-04-06-a.cpp -o 21B12111-04-06-a
// 実行方法：ターミナル上で.\21B12111-04-06-aを実行
/*結果
...
void init()
{
    ...
    primitives.push_back(one); primitives.push_back(alpha);
...
vector<vector<int>> createG(vector<vector<int>> alpha, vector<vector<int>> o)
{
    vector<vector<int>> res;
...
int main()
{
    ...
    G.push_back(tmp); //生成行列の第一行
    //G.push_back(primitives);    //生成行列の第二行
    //tmp = primitives;
    for (int i = 1; i < k; i++)
    {
        tmp = createG(tmp, primitives);
        G.push_back(tmp);
    }

    print_matrix(G);
    vector<vector<vector<int>>> cs = product(us, G);
    printf("OUTPUT:\n");
    printf("c=\n");
    for (auto c : cs)
    {
