...
INI, / initialize data
		LDA VH2			/ AC <- 0010
		IMK				/ IMSK <- 0010 (S_IN enabled)
		PIO				/ IOT <- 1 (serial-IO selected)
/ initialize
		CLA				/ AC <- 0
		STA BYE			/ M[BYE] <- 0
		...
		LDA CH_EQ		/ AC <- '='
		STA OPR			/ M[OPR] <- '='
		BSA INI_ST		/ call INI_ST (initialize state)ERR
...
////////// subroutine (write Z to ZMG) //////////
		LDA A_ZMG		/ AC <- M[A_ZMG] (ZMG)
		ADD VH3			/ AC <- ZMG + 3
		STA TMA			/ M[TMA] <- ZMG + 3	  M[TMA]: digit0
		LDA VM4			/ AC <- M[VM4] (-4)
		STA CNT			/ M[CNT] <- -4
PUT_DGT,
		LDA Z			/ AC <- M[Z]
